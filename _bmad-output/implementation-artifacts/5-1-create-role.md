...
2. **Given** the body is missing required fields or is invalid
   **When** the caller `POST /roles`
   **Then** the API returns `400` with the OA envelope

3. **Given** a role with that `name` already exists on client `oauth`
   **When** the caller `POST /roles`
...
- [ ] Task 1 — Client role create (AC: #1)
  - [ ] Resolve client UUID for `KEYCLOAK_CLIENT_ID` (`oauth`) via Admin API; create role on **that client**, not a realm role
  - [ ] Body at least `{ "name": "..." }`. README documents full representation (id, name, description, …)
  - [ ] Canonical names `administrator`, `coordinator`, `professor`, `student` (English B.2). **Never** `funcionario` / `coordenador`.
- [ ] Task 2 — 400 / 409 / Bearer (AC: #2–#4)
- [ ] Task 3 — Tests + README shape
- [ ] Do not implement list/get/put/patch/delete/assign in this PR

## Dev Notes

### Scope (this story only)

**In scope:** `POST /roles` as **client** roles on `oauth`.
...
**Out of scope:** 5.2–5.8, Epic 6 realm JSON, domain APIs.

Professor `constrsw.json` already has the four client roles — create API must still work for additional names and return 409 on duplicates.

### Anti-patterns

- Creating **realm** roles. Portuguese names. Using `realm-export.json`.
...
- [Source: `epics.md` — Story 5.1, FR11, FR13, NFR8]
- [Source: `oauth-api.md` — CAP-3; `keycloak-authz.md` — role names]
- [Source: Story 4.1 Bearer]

## Previous story intelligence

Reuse 4.1 admin client + Bearer. Look up client by `clientId=oauth`.

## Latest tech information

- Client roles: `POST /admin/realms/{realm}/clients/{id}/roles`.
