...
- [ ] Task 1 — `GET /roles/:id` (AC: #1–#3)
  - [ ] `id` is Keycloak role id (UUID) unless README pins name; stay consistent with 5.1 representation `id` field
  - [ ] Must still be a **client role on oauth** (404 if it is a realm role id)
- [ ] Task 2 — Bearer (AC: #4)
- [ ] Task 3 — Tests
...
**Out of scope:** PUT/PATCH/DELETE/assign.
...
- [Source: `epics.md` — Story 5.3, FR11, FR13]
- [Source: Story 5.1 — shape; 4.1 Bearer]

## Previous story intelligence

Reuse 5.1/5.2 client-role lookup. Keycloak also has `roles-by-id` Admin resource — use whatever returns the client role; document.

## Latest tech information

- Prefer `GET /admin/realms/{realm}/roles-by-id/{role-id}` then verify `clientRole` + container is oauth.
