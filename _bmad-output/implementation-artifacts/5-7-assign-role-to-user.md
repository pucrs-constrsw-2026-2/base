...
1. **Given** a valid Bearer token, an existing user, and an existing client role on `oauth`
...
3. **Given** the payload is invalid
   **When** the caller invokes assign
...
- [ ] Task 1 — Pin path (AC: #1)
  - [ ] Default from epics.md: **`POST /users/{{id}}/roles`**
  - [ ] Body documented (role `id` and/or `name`). Assign **client** role on `oauth`, not realm role
  - [ ] README pins path **once** — Story 5.8 must reuse it
- [ ] Task 2 — Keycloak role-mapping (AC: #1)
  - [ ] Admin: user client-level role mappings for the oauth client UUID
- [ ] Task 3 — 404 / 400 / Bearer
- [ ] Task 4 — Tests; do **not** implement unassign in this PR

## Dev Notes

### Scope (this story only)

**In scope:** assign only. Path pin is load-bearing.
...
**Out of scope:** 5.8 unassign (separate PR). Do not add `/roles/{id}/users` as a second URL.

If you deviate from `POST /users/{id}/roles`, README must be the single pin and 5.8 must follow — epics.md prefers the path above.
...
- [Source: `epics.md` — Story 5.7, FR12, FR13]
- [Source: `oauth-api.md` — assign path left to implementation, pinned here]
- [Source: Story 4.1 Bearer]

## Previous story intelligence

Users 4.x + roles 5.1–5.3 must exist. Client UUID lookup already in 5.1.

## Latest tech information

- `POST /admin/realms/{realm}/users/{id}/role-mappings/clients/{client-uuid}` with role representation list.
