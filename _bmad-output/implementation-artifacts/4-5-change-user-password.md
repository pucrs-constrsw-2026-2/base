...
3. **Given** the body is missing `password` or is invalid
   **When** the caller `PATCH /users/{{id}}`
...
- [ ] Task 1 — `PATCH /users/:id` password only (AC: #1, #3)
  - [ ] Body **must** be `{ "password": "..." }` for this story (do not turn PATCH into a generic user merge — that would collide with 4.4)
  - [ ] Keycloak: `PUT {adminRealmUrl}/users/{id}/reset-password` with `{ type: "password", value, temporary: false }` (or equivalent 26 API)
  - [ ] Success `200` empty body
- [ ] Task 2 — 404 / 400 / 401 / 403 OA (AC: #2–#4)
- [ ] Task 3 — Tests; do not log passwords
- [ ] README: PATCH is password-only

## Dev Notes

### Scope (this story only)

**In scope:** password PATCH only.
...
**Out of scope:** 4.6 soft-delete; 4.4 attributes; role assign.

### Anti-patterns

- Accepting `first-name` on this PATCH. Returning token after password change. Storing passwords locally.
...
- [Source: `epics.md` — Story 4.5, FR8, FR10]
- [Source: `oauth-api.md` — PATCH /users/{{id}}]
- [Source: Story 4.1 Bearer]

## Previous story intelligence

Reuse Bearer + admin client. Missing user → 404 before or via Keycloak.

## Latest tech information

- Keycloak 26 reset-password endpoint still lives under Admin users resource.
